...
가령 처음에 예시로 들었던 웹앱을 만든다고 가정을 한다면,  특정상황에 우리가 원하는 파일을 대신해서 보여준다거나 요청 혹은 완료된 네트워크 자원을 감시하여 특정한 이벤트를 발생시키는 등의 백그라운드에서 동작하는 작업들은 **"서비스워커"** 가 도맡아 처리합니다.
대부분의 백그라운드에서 동작을 하는 비즈니스 로직들은 서비스 워커가 처리를 하고 
...
> [목차](#목차)
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcxODUwNjA0OCw3MDk4NjE1NjksNTU2MT
